...
-(void)example
{
     ...
     *  1.显示启动页广告
     */
    [JDLaunchAd showInWindow:self.window AdFrame:CGRectMake(0, 0,self.window.bounds.size.width, self.window.bounds.size.height-0) setAdImage:^(JDLaunchAd *launchAd) {
...
//                                VC.URLString = openUrl;
                VC.view.backgroundColor = [UIColor redColor];
                                [self.window.rootViewController presentViewController:[[UINavigationController alloc]initWithRootViewController:VC] animated:NO completion:nil];
                
            }];
            return ;
            [launchAd setImageUrl:imgUrl duration:duration skipType:SkipTypeTimeText options:JDWebImageDefault completed:^(UIImage *image, NSURL *url) {
